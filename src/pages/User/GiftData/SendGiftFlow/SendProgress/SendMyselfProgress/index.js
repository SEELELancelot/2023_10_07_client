...
import {getSendProgressMyself} from "@/networkReuest/Myaxios";
import {Table} from "antd";

const SendMyselfProgress = () => {
  ...
  const fetchData = async () => {
    const result = await getSendProgressMyself();
    const {success, message} = result.data;
    console.log(success,message);
    if(success===1){
      setmyselfData(message);
      setIsLoading(false);
    }
  }
  const columns = [
  ...
  return (
    <div>
      {
        isLoading?<></>:
          <Table columns={columns} dataSource={myselfData}
...
export default SendMyselfProgress;
